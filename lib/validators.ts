...
const required = (label: string, min = 2) =>
  z.string().trim().min(min, `من فضلك اكتبي ${label} بشكل صحيح.`);
...
export const TeacherApplicationSchema = z.object({
  ...
  available_1_to_8: requiredChoice("التفرغ من 1 إلى 8").refine((v) => ["نعم", "لا", "تفرغ جزئي"].includes(v), {
  ...
  can_teach_tajweed: requiredChoice("تدريس التجويد").refine((v) => ["نظري", "عملي", "كلاهما"].includes(v), {
    message: "من فضلك اختاري (نظري/عملي/كلاهما)."
  }),
  can_teach_noor_al_bayan: requiredChoice("تدريس نور البيان").refine((v) => ["نعم", "لا"].includes(v), {
    message: "من فضلك اختاري نعم أو لا."
  }),
  ...
  preferred_students: requiredChoice("الفئة المفضلة").refine((v) => ["أطفال", "كبار", "كلاهما"].includes(v), {
...
export const stepFields = [
  ["agree_all_conditions", "available_1_to_8", "internet_type", "device_type", "can_use_tools", "agree_no_direct_contact"],
  ["full_name_3", "age", "marital_status", "whatsapp_number", "education", "finished_study"],
  ["ijazat_and_courses", "ijazah_hafs", "ijazah_tajweed", "can_teach_tajweed", "can_teach_noor_al_bayan", "other_subjects"],
  ["online_years", "kids_years", "good_with_kids", "teaching_age_from", "preferred_students", "academies_worked_with", "session_plan", "agree_no_stopping_students_policy"]
] as const;
